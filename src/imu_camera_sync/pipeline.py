...
def process_session(
    session_dir: str, method: str = "nearest", with_visualization: bool = False
) -> dict:
    ...
    try:
        ...
        synced = synchronizer.align(imu_clean, camera_clean, method=method)
...
def _stats_alignment(
    ...
    if idx is not None:
        deltas = np.abs(imu["timestamps"][idx] - cam["timestamps"]) * 1000
